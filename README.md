...
| [`flight-research`](skills/flight-research/SKILL.md) | Research price-first round-trip airfare with Google Flights discovery, SQLite run output, and carrier-site validation. Use only when origin IATA code, destination IATA code, passenger count, departure date, and return date are provided; date mode may be relative or exact and defaults to relative. |
