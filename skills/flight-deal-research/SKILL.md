...
Use `$browser:browser` for live research, rank by total price first, and front-load ambiguity because this skill often runs unattended in `/goal` mode.
...
3. Search normal booked itineraries first:
   - direct flights,
   - connecting flights booked together on the same carrier or alliance/interline itinerary,
   - major flight-search surfaces and airline sites where useful for validation.
4. Search separate-ticket permutations:
...
5. For international trips, broaden the middle leg:
...
6. Exclude any itinerary that requires an airport transfer. If a connection arrives at one airport and departs from another in the same city, discard it.
7. Apply connection buffers:
...
8. Price each candidate for the full party and baggage assumptions. Include unavoidable fees, separate-ticket totals, and currency conversion notes when relevant.
9. For separate-carrier itineraries, research each carrier's baggage policy before recommending the option:
...
10. Rank final candidates by price first. Also identify a "Best Recommended" option that considers fewer connections, no overnight layovers, same-ticket protection, baggage policy, and connection risk.
11. Validate the top results by re-checking the live page or carrier/OTA detail page near the end of the session. Flight prices change quickly; report the timestamp and any uncertainty.
